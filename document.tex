...
We tracked five named training runs across the two notebook generations and submitted five Kaggle predictions. Table~\ref{tab:runs} summarizes the configurations and Table~\ref{tab:lb} the leaderboard outcomes.
...
After Run E we performed a detailed analysis of where remaining errors live, comparing OCR and non-OCR submissions on the 134 rows where they disagree. The disagreements are heavily concentrated in five categories: \emph{Genes to traits} (Punnett squares, 49\% disagreement rate), \emph{Velocity, acceleration, and forces} (40\%), \emph{Solutions} (37\%), \emph{Astronomy} (30\%), and \emph{Magnets} (22\%). These are precisely the question types that require reading numerical labels off rendered diagrams --- exactly where a 500M-parameter VLM with a 384$\times$384 SigLIP encoder is weakest. We attempted higher-resolution inference (img\_size=768) but the resulting distribution shift hurt more than it helped.
...
\section*{Code (Github)}
The full source code, including both notebook generations, OCR preprocessing scripts, ensembling pipeline, and submission histories, is available at \url{https://github.com/tanish-tc/DeepLearning-final.git}.
